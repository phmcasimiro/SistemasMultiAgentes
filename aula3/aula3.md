...
## Mapa da Aula (Roadmap de Aprendizagem)

| # | Seção | Ideia central | Exercícios | Status | Arquivo(s) |
|---|---|---|---|---|---|
| 1 | O loop por baixo dos panos | Tornar o loop visível com hooks | 1.1, 1.2, 1.3 | ✅ | `agente_loop.py`, `agente_loop_max.py`, `agente_clima_vento.py` |
| 2 | Hooks (RunHooks × AgentHooks) | Referência de todos os `on_*` e a ordem de disparo | — (referência) | ✅ | `agente_hook1.py`, `agente_hook2.py`, `agente_hook3.py` |
| 3 | Critérios de parada | Três formas de dizer "pare" | 3.1, 3.2, 3.3 | ✅ | `agente_parada.py`, `agente_parada_timeout.py`, `agente_parada_dupla.py` |
| 4 | Retries e resiliência | Backoff, idempotência, dado real injetado | 4.1, 4.2 | ✅ | `agente_retry.py`, `agente_retry_idempotencia.py`, `agente_data.py` |
| 5 | Autonomia e freio humano | `needs_approval` — pausa até alguém decidir | 5.1, 5.2, 5.3 | ✅ | `agente_aprovacao.py` |
| 6 | Observando o loop ao vivo | Streaming — ver cada decisão/token em tempo real | 6.1, 6.2, 6.3 | ✅ | `agente_loop_stream.py` |
| 7 | Loop com decisões encadeadas | Múltiplas tools independentes, ordem decidida pelo modelo | 7.1, 7.2, 7.3 | ✅ | `agente_investigador.py` |

**Como usar este roadmap:** as 7 seções são cumulativas — a Seção 7 (decisões encadeadas) não traz nenhum mecanismo novo, é a SOMA das Seções 1, 4 e 5 aplicada a um cenário fiel ao trabalho da PCDF (ver Seção 7 abaixo). Todas as seções já estão implementadas; o roteiro de estudo sugerido (visto por primeira vez ou para revisão) é a ordem 1→7 da tabela.
...
> **Variante mais rígida do enunciado (slide 7):** o desafio original pede para, além do `max_turns`, forçar `ModelSettings(tool_choice="required", reset_tool_choice=False)` no `Agent` — isso IMPEDE de vez o modelo de responder em texto puro em qualquer turno, obrigando-o a sempre chamar alguma ferramenta (o mesmo mecanismo da Fase 4 da Aula 1, `agente4.py`, mas aplicado a **todos** os turnos via `reset_tool_choice=False`, não só o primeiro). `agente_loop_max.py` implementa a versão mais simples — só a instrução em texto + `max_turns` — o que já é suficiente para provar o ponto pedagógico central ("a instrução sozinha não é o que corta o loop"), mas sem o `tool_choice="required"` o modelo às vezes decide parar de chamar a ferramenta por conta própria antes do limite, tornando a demonstração um pouco menos determinística que a versão do slide.
...
**Pegadinha clássica de prova (slide 14):** os nomes NÃO são idênticos entre as duas famílias. Em `RunHooks`, o evento "um agente virou o ativo" é `on_agent_start(ctx, agent)`. Em `AgentHooks`, o mesmo evento (mas amarrado a UM agente específico) chama-se **`on_start(ctx, agent)`** — sem o `agent_` no meio. Copiar `on_agent_start` para dentro de uma classe `AgentHooks` não dá erro de sintaxe, mas o método nunca dispara (não existe esse hook nesse contexto). Da mesma forma, `on_handoff` inverte a ordem dos parâmetros entre as famílias: em `RunHooks` é `on_handoff(ctx, from_agent, to_agent)`; em `AgentHooks` (disparado no agente que RECEBE a transferência) é `on_handoff(ctx, agent, source)`, onde `source` é quem transferiu. `on_tool_start/end` e `on_llm_start/end` são as únicas assinaturas idênticas entre as duas famílias. *(arquivo: `agente_hook3.py` usa só os hooks compartilhados — `on_llm_start`, `on_tool_start/end` —, por isso não expõe essa pegadinha na prática; fique atento se for escrever um `AgentHooks` próprio.)*
